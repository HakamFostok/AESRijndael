...
internal class Word : IEnumerable<byte>
{
    ...
    internal static Word Xor(Word w1, Word w2) =>
        new(w1.Select((b, index) => Convert.ToByte(b ^ w2[index])).ToList());

    internal Word RotWord() =>
        new(this[1], this[2], this[3], this[0]);

    internal Word SubWord() =>
        new(this.Select(b => Table.SBox[b.GetUpperPart(), b.GetLowerPart()]).ToList());
