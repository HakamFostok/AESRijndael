...
class BinaryPolynomial : IEnumerable<int>
{
    ...
    public BinaryPolynomial(int number)
        : this()
    {
        BitArray temp = new(new int[] { number });
    ...
    public static BinaryPolynomial Xor(BinaryPolynomial left, BinaryPolynomial right) => new BinaryPolynomial
    {
        values =
    ...
    public static BinaryPolynomial ComplexOperation(int first, int second)
    {
        ...
        BinaryPolynomial result = Multiply(new BinaryPolynomial(first), new BinaryPolynomial(second));
        return ReminderConstant(result);
    }

    public static BinaryPolynomial Reminder(BinaryPolynomial up, BinaryPolynomial down)
    {
        BinaryPolynomial result = new() { values = new SortedSet<int>(up.Select(num => num)) };
    ...
    private static BinaryPolynomial ReminderConstant(BinaryPolynomial up) => Reminder(up, new BinaryPolynomial(283));

    public static BinaryPolynomial Multiply(BinaryPolynomial left, BinaryPolynomial righ) => left.Select(item => BinaryPolynomial.MultiplyByNumber(righ, item)).Aggregate((first, second) => BinaryPolynomial.Xor(first, second));

    public static BinaryPolynomial MultiplyByNumber(BinaryPolynomial left, int number) => new BinaryPolynomial { values = new SortedSet<int>(left.Select(num => num + number)) };
