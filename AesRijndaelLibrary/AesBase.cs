...
namespace AesRijndaelLibrary
{
    ...
    public abstract class AesBase
    {
        ...
        private static void InvShiftByte(byte[,] state)
        {
            GeneralShift(state, false);
        }

        private static void ShiftRow(byte[,] state)
        {
            GeneralShift(state, true);
        }
        ...
        private static void InvSubByte(byte[,] state)
        {
            GeneralSubByte(state, b => Table.InverseSBox[b.GetUpperPart(), b.GetLowerPart()]);
        }

        private static void SubByte(byte[,] state)
        {
            GeneralSubByte(state, b => Table.SBox[b.GetUpperPart(), b.GetLowerPart()]);
        }
